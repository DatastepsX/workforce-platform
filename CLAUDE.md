...
2. Deploy to production:
   ...
   - **Claude Code Mobile/Web sessions**: the project **is now** Git-connected (as of 2026-08-18) and merges to `main` trigger a real production deployment (`"source": "git"`, aliased straight to `workforce-platform-omega.vercel.app`) — confirmed by watching a deployment go `QUEUED → BUILDING → READY` within about a minute of a merge, with `age: 0` on the production URL afterward proving it wasn't a stale cached response. **This was NOT always true.** Earlier the same session, a merge (commit `be07462`) sat with no deployment at all for ~10 minutes, and the Vercel dashboard's Production Deployment panel showed an actionable "Connect Git" button — genuine proof the repo wasn't linked yet. The user tapped that button mid-session ("Connected just now," per their own dashboard screenshot, with Pull Request Comments + `deployment_status`/`repository_dispatch` webhooks enabled), and the very next merge deployed automatically. So: if a merge doesn't produce a deployment within a few minutes, **check the dashboard's Git connection panel first** — don't assume it's just slow, and don't assume a working deploy from one merge means it was always connected (this session drew both those wrong conclusions in turn before landing on the real explanation). After merging, poll `list_deployments`/`get_deployment` (or the dashboard) for a `READY` deployment on your commit SHA before telling the user something is live. The `deploy_to_vercel` MCP tool remains impractical here regardless (inline-file-tree-only, 222 files/2.5MB) — no need to reach for it.
3. Send the changelog email (see below). As of 2026-08-18, both env vars and network access work from Claude Code Mobile/Web sessions (see Mobile Workflow below) — use the standard Resend script pattern (same as Mac/VSCode). Only fall back to the Gmail MCP (`send_message`/`create_draft`) if the Resend call itself fails; that fallback path was still untested as of 2026-08-18 (Vercel MCP's equivalent "requires approval" hang was confirmed resolved, but Gmail MCP wasn't re-checked since Resend worked directly).
...
## Current Status
- **AI env var renamed** (2026-08-18): the app now reads `WORKFORCEX_ANTHROPIC_API_KEY` instead of `ANTHROPIC_API_KEY` in all 7 call sites (`src/lib/actions/{tenants,scenario-runs,submissions}.ts`, `src/app/api/{optimize-idea,fix-step,generate-test-data,career-avatar/generate}/route.ts`) — `ANTHROPIC_API_KEY` is reserved by the Claude Code platform for the coding session's own account auth and is silently never passed through to the app container when set under that name in a Claude Code environment's Umgebungsvariablen box. See Environment Variables section for the full explanation. **Confirmed working 2026-08-18** via a fresh session: `WORKFORCEX_ANTHROPIC_API_KEY` is now present in the container env and reaches Anthropic successfully. **Separate, unrelated blocker found**: the key itself has hit its Anthropic Console usage limit (`400 invalid_request_error`, resets 2026-09-01) — AI features will keep failing until the limit is raised/credit added, independent of anything in this repo or Claude Code environment settings.
...
- **Mobile Workflow**: two supported entry points, both sharing `SESSION_LOG.md` for continuity — (1) **Claude Code Mobile/Web sessions** (primary): work from the Claude Code mobile app or claude.ai/code; spins up an ephemeral remote container with `DatastepsX/workforce-platform` attached. Env vars (Supabase, `WORKFORCEX_ANTHROPIC_API_KEY`, `RESEND_API_KEY`, `NEXT_PUBLIC_APP_URL`) must be set as persistent **Environment Variables** in the Claude Code environment settings, not a git-tracked `.env.local` — the container is rebuilt from scratch each session, so `npm install` also reruns each time. **As of 2026-08-18, all 5 non-Anthropic vars are confirmed present and working** (`NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `RESEND_API_KEY`, `NEXT_PUBLIC_APP_URL` — `npm run dev` + `GET /login` returns 200, no more Supabase-init 500) — the root cause of the earlier 2026-08-15 failure was that the env vars had only ever been entered into the wrong UI (a placeholder-text box, or the general account Einstellungen modal rather than the environment's own Umgebungsvariablen box under Settings → Environments → Default). **The Anthropic key needed a second, separate fix, found 2026-08-18**: the user *had* correctly entered `ANTHROPIC_API_KEY` into the right Umgebungsvariablen box, but it still never appeared in the container `env` — because the Claude Code platform reserves that exact name for the coding session's own account auth (the environment settings UI shows an explicit warning to this effect) and doesn't pass a user-supplied value through under that name. Fixed by renaming the variable the app itself reads to `WORKFORCEX_ANTHROPIC_API_KEY` everywhere (see Environment Variables section below) — no platform-side collision, same key value. Supabase/Vercel/Gmail are connected as MCP integrations at the account level; Supabase MCP works reliably for DB reads/migrations even without app env vars. **This environment's network policy is confirmed fixed as of 2026-08-18** — `api.vercel.com` and `api.resend.com` both return real API responses now (not proxy 403s), and the earlier "Vercel MCP calls require approval and never resolve" issue is also resolved (`list_teams`/`list_projects`/`get_project`/`list_deployments` all return normally; the equivalent Gmail MCP hang was not re-tested since Resend worked directly). **Production deploys**: the project **is now** Git-connected (as of 2026-08-18) — merging to `main` triggers a real production deployment (`source: "git"`, aliased to `workforce-platform-omega.vercel.app`), confirmed by watching one go `QUEUED → BUILDING → READY` in about a minute and verifying `age: 0` on the production URL afterward. **The full story, since this session got it wrong twice before finding the truth**: it started genuinely NOT connected — a merge earlier the same session (commit `be07462`) sat with zero deployment activity for ~10 minutes, and the Vercel dashboard's Production Deployment panel showed an actionable "Connect Git" button, real proof of no link. That was the correct read. Then a second merge appeared to deploy automatically minutes later, which looked like it disproved the "not connected" theory — so the docs were flipped to claim the project had been Git-connected all along. That was wrong too: the real explanation, confirmed by the user's own dashboard screenshot ("Connected just now," with PR Comments + `deployment_status`/`repository_dispatch` webhooks enabled), is that the user tapped "Connect Git" themselves in the middle of the session, and that's what made the second merge deploy. **Lesson: when a merge doesn't produce a deployment within a few minutes, check the dashboard's Git connection panel directly — don't infer connection status from metadata fields or from one merge's success/failure alone**, since both directions of that inference were wrong here. The `deploy_to_vercel` MCP tool (inline file-tree upload, impractical for a 222-file/2.5MB app) is still not the right tool here. **Changelog email**: works via the standard Resend script pattern when network + `RESEND_API_KEY` are good, though one send attempt later in the 2026-08-18 session was blocked by this session's own permission classifier for unclear reasons — not a network/key issue, retry if it recurs. (2) **iPhone SSH** (alternative, requires Mac powered on): via Termius + Tailscale (100.111.139.5) into the Mac; Claude Code CLI on subscription billing (Sonnet 4.6)
...
WORKFORCEX_ANTHROPIC_API_KEY=...        # Required for AI test data generation
...
**Why `WORKFORCEX_ANTHROPIC_API_KEY` and not `ANTHROPIC_API_KEY`** (renamed 2026-08-18): `ANTHROPIC_API_KEY` is reserved by the Claude Code platform itself — it authenticates the coding session against your Anthropic account, and the environment settings UI explicitly warns it "wird nicht zur Authentifizierung von Anfragen verwendet" (is not used to authenticate requests) when you type that exact name into a Claude Code environment's Umgebungsvariablen box. In practice this means a user-supplied `ANTHROPIC_API_KEY` set there never reaches the app's container `env` — confirmed missing across five separate verification attempts on 2026-08-09/15/18, while every other var worked. The app now reads `WORKFORCEX_ANTHROPIC_API_KEY` everywhere instead, which has no collision with the platform's reserved name. Same key value, different variable name — used consistently on both Mac/VSCode `.env.local` and Claude Code Mobile/Web environment settings to avoid drift.

...
2. Deploy to production:
   ...
   - **Claude Code Mobile/Web sessions**: the project **is** Git-connected and merges to `main` do trigger a real production deployment (`"source": "git"`, aliased straight to `workforce-platform-omega.vercel.app`) — confirmed 2026-08-18 by watching a deployment go `QUEUED → BUILDING → READY` within about a minute of a merge, with `age: 0` on the production URL afterward proving it wasn't a stale cached response. **Known anomaly, unexplained**: on the same day, one specific merge (commit `be07462`) did not visibly trigger a deployment even after ~10 minutes of polling — the very next merge (`d6fdc39`, built on top of it) deployed normally and carried both commits' changes live, so the practical impact was nil, but don't assume every merge deploys within a fixed time window. After merging, poll `list_deployments`/`get_deployment` (or check the Vercel dashboard) for a `READY` deployment on your commit SHA before telling the user something is live — don't assume it from the merge alone. The `deploy_to_vercel` MCP tool remains impractical here regardless (inline-file-tree-only, 222 files/2.5MB) — no need to reach for it.

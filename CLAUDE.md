...
2. Deploy to production:
   ...
   - **Claude Code Mobile/Web sessions**: this environment's network policy blocks direct calls to `api.vercel.com` (confirmed still true 2026-08-15, even after raising the network access tier in environment settings), and the Vercel MCP `deploy_to_vercel` tool only supports uploading a full file tree inline (impractical for this app's size — 222 files / 2.5MB) — plus, as of 2026-08-15, Vercel MCP calls started returning "MCP tool call requires approval" with no way to approve from inside the session. **No working deploy path currently exists from this environment.** Skip this step here and note in `SESSION_LOG.md` that a Mac/VSCode session needs to deploy.
3. Send the changelog email (see below) — **Claude Code Mobile/Web sessions**: `api.resend.com` is also blocked by this environment's network policy (confirmed still true 2026-08-15). The Gmail MCP fallback (`send_message`/`create_draft`) was expected to work instead but as of 2026-08-15 both also return "MCP tool call requires approval" and never resolve. **No working changelog-email path currently exists from this environment either.** Skip this step and note it in `SESSION_LOG.md` instead of retrying.
...
- **Mobile Workflow**: two supported entry points, both sharing `SESSION_LOG.md` for continuity — (1) **Claude Code Mobile/Web sessions** (primary): work from the Claude Code mobile app or claude.ai/code; spins up an ephemeral remote container with `DatastepsX/workforce-platform` attached. Env vars (Supabase, `ANTHROPIC_API_KEY`, `RESEND_API_KEY`, `NEXT_PUBLIC_APP_URL`) must be set as persistent **Environment Variables** in the Claude Code environment settings, not a git-tracked `.env.local` — the container is rebuilt from scratch each session, so `npm install` also reruns each time. **As of 2026-08-15, adding these vars to the environment settings did not make them appear in the session container** (`env | grep` found none of them) — root cause unconfirmed, don't assume the box is wired up until re-verified in a fresh session. Supabase/Vercel/Gmail are connected as MCP integrations at the account level; Supabase MCP works reliably for DB reads/migrations even without app env vars. **This environment's network policy still blocks direct outbound calls to `api.vercel.com` and `api.resend.com`** as of 2026-08-15 — confirmed via 403 on the agent proxy even after the user raised the environment's network access tier, so that setting change alone doesn't appear to lift this block either. There is currently no working production deploy path from this environment (the Vercel MCP `deploy_to_vercel` tool only supports uploading a full file tree inline, impractical for a 222-file/2.5MB app, and as of 2026-08-15 `list_projects`/other Vercel MCP calls also started returning "MCP tool call requires approval" with no way to grant it inside the session); deploys still need to happen from a Mac/VSCode session. The changelog email was meant to go via the Gmail MCP integration instead of Resend, but as of 2026-08-15 both `send_message` and `create_draft` also return "MCP tool call requires approval" and never resolve — so from this environment, in its current state, neither the deploy step nor the changelog email step has a working path; skip both and log it in `SESSION_LOG.md` rather than retrying. (2) **iPhone SSH** (alternative, requires Mac powered on): via Termius + Tailscale (100.111.139.5) into the Mac; Claude Code CLI on subscription billing (Sonnet 4.6)

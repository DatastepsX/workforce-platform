...
2. Deploy to production:
   ...
   - **Claude Code Mobile/Web sessions**: as of 2026-08-18 the `workforce-platform` Vercel project is confirmed **Git-connected** — every recent production deployment carries `githubCommitRef: main` metadata, meaning it **auto-deploys whenever a commit lands on `main`**. There is no separate manual deploy action to take from this environment: get the session's branch/PR merged into `main` and Vercel deploys it automatically (the `deploy_to_vercel` MCP tool's inline-file-tree-only upload was never actually the right tool here — it's for repo-less projects — skip it). If this session's own changes are docs-only or on a non-`main` branch, no deploy is expected or needed; note that plainly in `SESSION_LOG.md` rather than treating it as a skipped step.

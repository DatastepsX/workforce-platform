...
2. Deploy to production:
   ...
   - **Claude Code Mobile/Web sessions**: this environment's network policy blocks direct calls to `api.vercel.com`, and the Vercel MCP `deploy_to_vercel` tool only supports uploading a full file tree inline (impractical for this app's size — 222 files / 2.5MB). **No working deploy path currently exists from this environment.** Skip this step here and note in `SESSION_LOG.md` that a Mac/VSCode session needs to deploy, unless the environment's network policy is changed to allow `api.vercel.com`.
3. Send the changelog email (see below) — **Claude Code Mobile/Web sessions**: `api.resend.com` is also blocked by this environment's network policy; use the Gmail MCP integration instead (already connected) rather than the Resend script.
...
- **Mobile Workflow**: two supported entry points, both sharing `SESSION_LOG.md` for continuity — (1) **Claude Code Mobile/Web sessions** (primary): work from the Claude Code mobile app or claude.ai/code; spins up an ephemeral remote container with `DatastepsX/workforce-platform` attached. Env vars (Supabase, `ANTHROPIC_API_KEY`, `RESEND_API_KEY`, `NEXT_PUBLIC_APP_URL`) must be set as persistent **Environment Variables** in the Claude Code environment settings, not a git-tracked `.env.local` — the container is rebuilt from scratch each session, so `npm install` also reruns each time. Supabase/Vercel/Gmail are connected as MCP integrations at the account level, so DB reads/migrations work via Supabase MCP tools even before app env vars are set. **This environment's network policy blocks direct outbound calls to `api.vercel.com` and `api.resend.com`** — the Vercel CLI and the Resend changelog script cannot reach their APIs from here. There is currently no working production deploy path from this environment (the Vercel MCP `deploy_to_vercel` tool only supports uploading a full file tree inline, impractical for a 222-file/2.5MB app); deploys still need to happen from a Mac/VSCode session unless the network policy is changed to allow `api.vercel.com`. The changelog email can go out via the Gmail MCP integration instead of Resend. (2) **iPhone SSH** (alternative, requires Mac powered on): via Termius + Tailscale (100.111.139.5) into the Mac; Claude Code CLI on subscription billing (Sonnet 4.6)

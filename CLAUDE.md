...
2. Deploy to production:
   ...
   - **Claude Code Mobile/Web sessions**: **CORRECTED 2026-08-18 (same day, later in the session) — the project is NOT actually Git-connected.** An earlier pass this session wrongly concluded it was, based on `githubCommitRef`/`githubOrg`/`githubRepo` fields present on every deployment record — but those fields are also attached whenever `vercel deploy --prod` is run manually from inside a git working directory (which is exactly what every Mac/VSCode session has always done), so their presence is not evidence of webhook auto-deploy. Confirmed via the actual Vercel dashboard: the Production Deployment panel shows a **"Connect Git"** button (proof no repo is linked), and every deployment in the list — including several same-day retries — is still the same old commit from Jul 16, with nothing appearing for any push made across this entire session despite ~10 minutes of polling. **There is currently no working auto-deploy path from Claude Code Mobile/Web sessions.** The `deploy_to_vercel` MCP tool remains impractical (inline-file-tree-only, 222 files/2.5MB), and an attempt to trigger a deploy via the `create_git_project` Vercel MCP tool was blocked by this session's own permission classifier. Until the user connects the repo via that "Connect Git" button (which should also enable real auto-deploy going forward), skip this step from mobile/web and note it plainly in `SESSION_LOG.md` — do not claim a deploy happened without confirming a fresh deployment ID in the Vercel dashboard first.
